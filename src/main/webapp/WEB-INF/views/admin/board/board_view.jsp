...
<script>
	...
	var printPageVO = function(pageVO, target) {
		...
		if(pageVO.prev){
		 ...
		 if(pageVO.next){
				paging = paging + '<li class="page-item"><a class="page-link" href="'+pageVO.endPage+1+'">이전</a></li>';													
			}
		target.html(paging);												
	}
	function getPage(pageInfo) {
		$.getJSON(pageInfo, function(data){
			...
			$("#modifyModal").modal('hide'); //수정, 삭제시 모달창 없애기
		});
	}
	...
	$(document).ready(function(){
		...
		$(".pagination").on("click", "li a", function(event){
			event.preventDefault();//기본 a href 이동 이벤트를 prevent(중지)
...
<script>
//댓글 입력/수정/삭제 버튼 처리(AJAX URL호출-JSON데이터 getset)
$(document).ready(function() {
	
	$("#replyModBtn").on("click",function(){
		var replytext = $("#replytext").val();
		var rno = $("#rno").val();
		//alert(replytext + rno); //디버그 : 입력값 확인
		//return false;
		$.ajax({
			type:'put',
